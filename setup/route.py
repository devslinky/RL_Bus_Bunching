...
class Route_Schema(ABC):
    ...
    def __init__(self) -> None:
        ...
        # Get bus capacity (can be overridden by subclasses)
        self._route_bus_capacity: Dict[str, int] = self._define_bus_capacity()
        ...
        for route_id in self._route_ids:
            route_details = Route_Details(
                ...
                bus_capacity=self._route_bus_capacity.get(route_id, INF),
    ...
    def _define_bus_capacity(self) -> Dict[str, int]:
        """Define bus capacity for each route.

        Override this method to set finite bus capacity.
        Default returns INF (unlimited capacity) for all routes.

        Returns:
            Dict mapping route_id to bus capacity (max passengers)
        """
        return {route_id: INF for route_id in self._route_ids}
