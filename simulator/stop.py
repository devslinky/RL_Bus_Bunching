...
class Stop(ABC):
    ...
    def take_snapshot(self) -> StopSnapshot:
        ...
        pax_arrival_times = self._pax_queue.get_pax_arrival_times()
        rejected_pax_count = self._pax_queue.get_rejected_pax_count()
    ...
    def get_rejection_events(self) -> List[Tuple[str, str, str, int, int]]:
        """Get rejection events with stop_id prepended.

        Returns:
            List of (stop_id, bus_id, route_id, time, num_rejected)
        """
        return [
            (self._stop_id, bus_id, route_id, t, num_rejected)
            for bus_id, route_id, t, num_rejected in self._pax_queue.get_rejection_events()
        ]
