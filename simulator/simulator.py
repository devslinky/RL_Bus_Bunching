...
class Simulator:
    ...
    def __init__(self, blueprint: Blueprint, agent: Agent, run_config: Dict) -> None:
        ...
        # Fleet size control
        # fleet_size: maximum number of buses allowed on route (None = unlimited)
        self._fleet_size = run_config.get('fleet_size', None)
        # dispatch_headway: minimum time between dispatches (default from route schema)
        self._dispatch_headway = run_config.get('dispatch_headway', None)
        # If not specified, get from first route's schedule_headway
        if self._dispatch_headway is None:
            for route in self._blueprint.route_schema.route_details_by_id.values():
                self._dispatch_headway = int(route.schedule_headway)
                break

        self._bus_in_running: Dict[str, Bus] = {}
        self._last_dispatch_time = 0

        # Warm-up to pre-distribute buses along the route
        self._warm_up_enabled = run_config.get('warm_up', False)
        print(f"Warm-up enabled: {self._warm_up_enabled}")
        if self._warm_up_enabled:
            print("Warm-up enabled. Warming up the simulation...")
            self._warm_up()
    ...
    @property
    def rejection_events(self) -> List[Tuple[str, str, str, int, int]]:
        """Get all rejection events from all stops.

        Returns:
            List of (stop_id, bus_id, route_id, time, num_rejected)
        """
        events = []
        for stop in self._stops.values():
            events.extend(stop.get_rejection_events())
        return events

    @property
    def left_paxs(self) -> List[Pax]:
        ''' Get all the passengers that have finished their trips.

        '''
        return self._left_paxs
    
    def _warm_up(self) -> None:
        """Warm up the environment by dispatching and distributing buses across the route.

        Runs simulation steps until the fleet reaches the target size and buses are
        spread out along the route (not just dispatched from terminal).

        During warm-up:
        - Buses are dispatched and moved along the route
        - No passengers are generated (to avoid affecting metrics)
        - Holding actions are set to 0 (buses don't hold during warm-up)
        """
        if self._fleet_size is None:
            return  # No warm-up needed for unlimited fleet

        print(f"Warm-up: Initializing fleet of {self._fleet_size} buses...")

        # Calculate warm-up duration: need enough time to dispatch all buses
        # and have them distributed along the route
        # Time to dispatch all buses = (fleet_size - 1) * dispatch_headway
        dispatch_time_needed = (self._fleet_size - 1) * self._dispatch_headway

        # Get approximate route travel time for better distribution
        route_travel_time = 0
        for route in self._blueprint.route_schema.route_details_by_id.values():
            # Estimate from number of stops and typical travel time
            num_stops = len(route.visit_seq_stops)
            route_travel_time = num_stops * 120  # ~2 min per stop segment
            break

        # Warm-up until all buses are dispatched and have had time to spread out
        # Add half the route travel time to allow buses to distribute
        warm_up_duration = dispatch_time_needed + min(route_travel_time // 2, 1800)

        print(dispatch_time_needed, route_travel_time, warm_up_duration)

        # Track dispatches during warm-up (use offset to avoid negative time issues)
        warm_up_offset = warm_up_duration
        dispatched_count = 0

        for step in range(warm_up_duration):
            # Use offset time for internal tracking
            t = step
            ...
            if fleet_ok and headway_ok:
                for terminal_id, terminal in self._terminals.items():
                    dispatching_buses = terminal.dispatch(t, force_dispatch=True)
                    if len(dispatching_buses) > 0:
                        self._last_dispatch_time = t
                        dispatched_count += len(dispatching_buses)
                    self._mediator.transfer(
                        dispatching_buses, 'terminal', terminal_id, t)
                    for bus in dispatching_buses:
                        print(f"Warm-up: Dispatched bus {bus.bus_id} at time {t}")
                        self._total_buses.append(bus)
                        self._bus_in_running[bus.bus_id] = bus

            # Run link operations (buses move along links)
            for link_id, link in self._links.items():
                leaving_link_buses = link.forward(t)
                self._mediator.transfer(leaving_link_buses, 'link', link_id, t)

            # Run stop operations (buses dwell at stops) - no passengers during warm-up
            for stop_id, stop in self._stops.items():
                leaving_stop_buses, _ = stop.operation(t)
                self._mediator.transfer(leaving_stop_buses, 'stop', stop_id, t)

            # Run holder operations with zero holding time during warm-up
            # First set all pending hold actions to 0
            empty_hold_action: Dict[Tuple[str, str, str], float] = {}
            self._holder.set_hold_action(empty_hold_action)
            ...
            # Update bus status - handle finished buses
            for bus in self._total_buses:
                if bus.status == 'finished':
                    if bus.bus_id in self._bus_in_running:
                        del self._bus_in_running[bus.bus_id]

        # Reset last dispatch time for proper headway tracking in main simulation
        self._last_dispatch_time = 0

        print(f"Warm-up complete: {len(self._bus_in_running)} buses running, {dispatched_count} total dispatched")
    ...
    def step(self, t: int, stop_bus_hold_time: Dict[Tuple[str, str, str], float]) -> Snapshot:
        ...
        # Check fleet size constraint and headway constraint
        fleet_ok = self._fleet_size is None or len(self._bus_in_running) < self._fleet_size
        headway_ok = (t - self._last_dispatch_time >= self._dispatch_headway) or (t == self._dispatch_headway)
        ...
        for bus in self._total_buses:
            if bus.status != 'finished':
                ...
                # Remove finished bus from running list
                if bus.bus_id in self._bus_in_running:
                    del self._bus_in_running[bus.bus_id]
