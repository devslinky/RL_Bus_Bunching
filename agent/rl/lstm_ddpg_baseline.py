"""
LSTM_DDPG_Baseline — DDPG with LSTM encoder using the SAME state features
as the Transformer agent (SetDDPG_BusHoldingAgent_Or).

Purpose: Compare recurrent sequence modelling (LSTM) against self-attention
(Transformer) for encoding variable-length leader bus sequences.

Architecture:
    ego → BusTokenEmbedding → ego_token
    leaders → BusTokenEmbedding → LSTM (packed) → last hidden state
    concat(ego_token, lstm_output) → LayerNorm → fused representation (token_dim)
...
# ──────────────────────────────────────────────
# LSTM Encoder (replaces SetLeaderEncoder)
# ──────────────────────────────────────────────

class LSTMLeaderEncoder(nn.Module):
    """
    LSTM-based encoder:
        ego → BusTokenEmbedding → ego_token
        leaders → BusTokenEmbedding → packed LSTM → last hidden state
        concat(ego_token, lstm_hidden) → fuse → output (token_dim)

    When no leaders are present, uses a learned fallback token.
    """

    def __init__(self, num_stops, ego_cont_dim, leader_cont_dim,
                 stop_emb_dim=32, token_dim=64, lstm_layers=2,
    ...
    def forward(self, ego_features, leader_features=None, num_leading=None):
        ...
        if leader_features is None or num_leading is None or int(num_leading.max().item()) == 0:
            ...
            lengths = num_leading.clamp(min=1).cpu()
            packed = nn.utils.rnn.pack_padded_sequence(
                leaders_tok, lengths, batch_first=True, enforce_sorted=False
            )
            _, (h_n, _) = self.lstm(packed)
            leader_repr = h_n[-1]  # Last layer hidden state [B, token_dim]

        return self.fuse(torch.cat([ego_tok, leader_repr], dim=-1))
...
# ──────────────────────────────────────────────
# Main Agent — LSTM_DDPG_Baseline
# ──────────────────────────────────────────────

class LSTM_DDPG_Baseline(RLAgent):
    ...
    def __init__(self, agent_config: Dict[str, Any], blueprint: Blueprint,
        ...
        print(f"LSTM_DDPG_Baseline: Using {self._device}")
        ...
        lstm_layers = agent_config.get("lstm_layers", 2)
        ...
        # Networks — LSTM encoder
        def make_encoder():
            return LSTMLeaderEncoder(
                num_stops, ego_cont_dim, leader_cont_dim,
                stop_emb_dim, token_dim, lstm_layers,
        ...
        print(f"LSTM_DDPG_Baseline: Actor params: {total:,}")
