...
class Rich_DDPG(RLAgent):
    ...
    def __init__(self, agent_config: Dict[str, Any], blueprint: Blueprint, run_config: Dict[str, Any]) -> None:
        ...
        # _reward_features_dict = agent_config.get('reward_features', ['headway_deviation'])
        # _state_features_dict = agent_config.get('state_features', ['normalized_headway', 'epsilon_arrival'])

        # self._state_size = len(_state_features_dict)
        self._state_size = 4  # Force state size to 4 to include pax_waiting and pax_waiting_next_stop
        ...
        self._w_pax_wait = agent_config.get('w_pax_wait', 0.5)  # Passenger wait penalty weight
        self._w_headway = agent_config.get('w_headway', 0.5)    # Headway deviation weight
    ...
    def _get_state_features(self, snapshot: Snapshot, acting_bus: Tuple[str, str], stop_id: str) -> List[float]:
        ...
        epsilon_arrival_curr_stop, _ = snapshot.get_bus_epsilon(route_id, bus_id, stop_id)
        ...
        curr_index = visit_seq_stops.index(stop_id)     
        if curr_index < len(visit_seq_stops) - 1:
            next_stop_id = visit_seq_stops[curr_index + 1]
        else:        
            next_stop_id = None  # This is the last stop
        pax_waiting_next_stop = snapshot.stop_snapshots[next_stop_id].pax_num if next_stop_id else 0

    
        return [normalized_headway, epsilon_arrival_curr_stop, pax_waiting, pax_waiting_next_stop]
    ...
    def _transform_snapshot_to_SR(self, snapshot: Snapshot, acting_bus: Tuple[str, str],
        ...
        State: [normalized_headway, epsilon_arrival, pax_waiting]
        ...
        # state = [normalized_headway, epsilon_arrival_curr_stop]
        state = self._get_state_features(snapshot, acting_bus, stop_id)
