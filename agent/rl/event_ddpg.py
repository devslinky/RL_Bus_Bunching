...
class Event_DDPG(RLAgent):
    ...
    def learn(self):
        ...
        for _ in range(5):
            ...
            Q_for_actor = self._ego_critic_net(s_imagi_a)
            ...
            self._record_metric('target_Q', float(total_target.mean().item()))
    ...
    def _get_additional_metrics(self) -> Dict[str, float]:
        """返回Event_DDPG特有的额外指标"""
        metrics = {}
        metrics['buffer/size'] = len(self._replay_buffer)
        metrics['buffer/capacity'] = self._replay_buffer.maxlen
        metrics['buffer/utilization'] = len(self._replay_buffer) / self._replay_buffer.maxlen
        metrics['explore/noise_level'] = self._noise_level
        metrics['train/learn_count'] = self._learn_count
        metrics['train/total_events'] = len(self._total_events)
        return metrics
    ...
    def _get_model_state_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'ego_critic_net': self._ego_critic_net.state_dict(),
            'event_critic_net': self._event_critic_net.state_dict(),
        }
    ...
    def _get_optimizer_state_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'ego_critic_optim': self._ego_critic_optim.state_dict(),
            'event_critic_optim': self._event_critic_optim.state_dict(),
        }
    ...
    def _get_target_model_state_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'target_ego_critic_net': self._target_ego_critic_net.state_dict(),
            'target_event_critic_net': self._target_event_critic_net.state_dict(),
        }

    def _load_model_state_dict(self, state_dict: Dict[str, Any]) -> None:
        """加载所有模型的状态字典"""
        self._actor_net.load_state_dict(state_dict['actor_net'])
        if 'ego_critic_net' in state_dict:
            self._ego_critic_net.load_state_dict(state_dict['ego_critic_net'])
        if 'event_critic_net' in state_dict:
            self._event_critic_net.load_state_dict(state_dict['event_critic_net'])
    ...
    def _load_optimizer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        if 'ego_critic_optim' in state_dict:
            self._ego_critic_optim.load_state_dict(state_dict['ego_critic_optim'])
        if 'event_critic_optim' in state_dict:
            self._event_critic_optim.load_state_dict(state_dict['event_critic_optim'])
    ...
    def _load_target_model_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        if 'target_ego_critic_net' in state_dict:
            self._target_ego_critic_net.load_state_dict(state_dict['target_ego_critic_net'])
        if 'target_event_critic_net' in state_dict:
            self._target_event_critic_net.load_state_dict(state_dict['target_event_critic_net'])
