...
class Pax_Wait_Time_DDPG(RLAgent):
    """
    ...
    def _transform_snapshot_to_SR(self, snapshot: Snapshot, acting_bus: Tuple[str, str],
        ...
        if snapshot.t % 100 == 0:  # Print debug info every 100 time steps
            print(f"Reward calc - headway: {headway:.1f}s, headway_reward: {headway_reward:.4f}, "
                f"pax_waiting: , pax_penalty: {pax_wait_penalty:.4f} at time {snapshot.t}")
