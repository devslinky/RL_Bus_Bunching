"""
MLP_DDPG_Baseline — DDPG with MLP encoder using the SAME state features
as the Transformer agent (SetDDPG_BusHoldingAgent_Or).

Purpose: Isolate the transformer architecture contribution by showing
what an MLP can achieve with identical input information.

Architecture:
    ego → BusTokenEmbedding → ego_token  (token_dim)
    leaders → BusTokenEmbedding → pad to max_leaders → flatten  (max_leaders * token_dim)
    concat(ego_token, leader_flat) → MLP → state representation  (token_dim)
...
# ──────────────────────────────────────────────
# Replay structures (same as transformer agent)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Embedding: single bus token (same as transformer)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# MLP Encoder (replaces SetLeaderEncoder)
# ──────────────────────────────────────────────

class MLPLeaderEncoder(nn.Module):
    """
    Flatten-based MLP encoder:
        ego → BusTokenEmbedding → ego_token
        leaders → BusTokenEmbedding → zero-pad to max_leaders → flatten
        concat → MLP → output (token_dim)
    """
    ...
    def __init__(self, num_stops, ego_cont_dim, leader_cont_dim,
        ...
        input_dim = token_dim + max_leaders * token_dim
        self.mlp = nn.Sequential(
            nn.Linear(input_dim, token_dim * 4),
            nn.ReLU(),
    ...
    def forward(self, ego_features, leader_features=None, num_leading=None):
        ...
        leader_flat = torch.zeros(B, self.max_leaders * self.token_dim,
                                  device=device)

        if leader_features is not None and num_leading is not None and int(num_leading.max().item()) > 0:
            ...
            # Fill into fixed-size buffer
            actual_k = min(K, self.max_leaders)
            leader_flat[:, :actual_k * self.token_dim] = leaders_tok[:, :actual_k].reshape(B, -1)

        x = torch.cat([ego_tok, leader_flat], dim=-1)
        return self.mlp(x)


# ──────────────────────────────────────────────
# Actor / Critic (same structure as transformer agent)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Main Agent — MLP_DDPG_Baseline
# ──────────────────────────────────────────────

class MLP_DDPG_Baseline(RLAgent):
    ...
    def __init__(self, agent_config: Dict[str, Any], blueprint: Blueprint,
        ...
        print(f"MLP_DDPG_Baseline: Using {self._device}")
        ...
        # Dimensions (same as transformer)
        ego_cont_dim = 4
        leader_cont_dim = 5
        ...
        # Networks — MLP encoder
        def make_encoder():
            return MLPLeaderEncoder(
        ...
        print(f"MLP_DDPG_Baseline: Actor params: {total:,}")
    ...
    # ──────────────────────────────────────────
    # Interaction (same as transformer)
    # ──────────────────────────────────────────
    ...
    # ──────────────────────────────────────────
    # Build transitions (same as transformer)
    # ──────────────────────────────────────────
    ...
    # ──────────────────────────────────────────
    # DDPG learning (same as transformer)
    # ──────────────────────────────────────────
